  ...
  return (
    <Box as="sibedar" style={{ display: "block" }}>
      ...
      <p>
        ...
        <a className="boxLink" style={{ color: "#000"}} href={`https://github.com/${propriedades.githubUser}`} target="_blank">
    ...
    <ProfileRelationsBoxWrapper>
      ...
      <ul>
        {props.itens.map((itemAtual) => {
          return (
            <li key={itemAtual.id}>
              <a href={itemAtual.login ? `/user/${itemAtual.login}` : itemAtual.url} target={itemAtual.login ? '_self' : '_blank'}>
...
export default function User(props) {  
  ...
  React.useEffect(() => {
    document.title = `@${githubUser} - GitKut - O Lugar da Nostalgia`
 }, []);
              ...
              <div>
                <label>
                  <span>
                    ...
                    (compativel com scrabs, copie o códiga desse site aqui <a href="http://www.scrapsweb.com.br/" target="_blank">www.scrapsweb.com.br</a>)
                </label>
...
export async function getServerSideProps(context) {
  ...
  const {githubUser} = context.query
  return {
    props: {
      githubUser
    },
